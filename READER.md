# Module 5B — Complete Reader

This document combines the complete contents of all Markdown files currently in the repository. It is formatted for reading and PDF export.
...
2. **Define Arrays**: Define two NumPy arrays, `x` and `y`, with the same shape.
3. **Use Boolean Indexing**:
   - `x > y` gives a Boolean array where elements of `x` are greater than `y`.
   - `x == y` gives a Boolean array where elements of `x` are equal to `y`.
4. **Find Indices**: Use `np.where()` to get the indices where the conditions are satisfied.
5. **Print Indices**: Print the indices where the conditions hold true.
...
3. **Delete Column**: Use `np.delete()` to remove the second column, with index `1`.
4. **Insert Column**: Use `np.insert()` to insert the new column at the second column’s original position.
5. **Display Result**: Print the updated array.
...
2. Create a dictionary named `exam_data` with the keys `name`, `score`, `attempts`, and `qualify`.
3. Create a list of custom index labels named `labels`.
...
## Result
Thus, the Python program has been created and executed successfully to create and display a DataFrame using the given dictionary and index labels.
...
## 🎯 Aim
To write a Python program using Pandas to **join two DataFrames along rows** through row-wise concatenation.

## 🧠 Algorithm
1. Import the `pandas` library.
2. Create the first DataFrame using `student_data1`.
3. Create the second DataFrame using `student_data2`.
4. Concatenate both DataFrames row-wise with `pd.concat()`.
...
## Result
Thus, the Python program has been created and executed successfully to join the two DataFrames row-wise using `pd.concat()`, including all records from both DataFrames in the final result.

---

# Conclusion

This reader combines all five Module 5B exercises covering NumPy arrays and Pandas DataFrames. It is ready to open in a Markdown viewer and export as a PDF.
